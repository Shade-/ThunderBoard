...
// Reload rules
	...
	if ($mybb->request_method == 'post') {
		...
		foreach ($_rules as $rule) {
			...
			list($file, $function) = explode('=', $rule, 2);
		...
		admin_redirect(MAINURL . '&action=reload');
...
function update_templates_dateline()
{
	global $cache;

	$pluginsCache = $cache->read('shade_plugins');
    $pluginsCache['ThunderBoard']['templates_last_updated'] = TIME_NOW;
